...
func init() {
	providers.RegisterAdminRoutes(map[string]providers.RouteHandler{
		"POST /auth/opencodezen/import": importOpenCodeZen,
	})
}

// importAccount mirrors the request body shape. Both `access_token` and a
// bare string per line are accepted.
...
// importOpenCodeZen handles POST /auth/opencodezen/import. Accepts:
//   - a single importAccount JSON object
//   - a JSON array of importAccount
//   - newline-delimited bare API keys (one per line)
//
// An empty access token enables the free tier (Bearer public upstream).
func importOpenCodeZen(host providers.Host, w http.ResponseWriter, r *http.Request) {
	...
	if len(entries) == 0 {
		writeJSONError(w, http.StatusBadRequest, "no opencodezen entries found")
		return
	}
	...
	for i, e := range entries {
		...
		if err := host.RefreshAccountModels(&account); err != nil {
			logger.Warnf("[opencodezen] model refresh failed for %s: %v", account.Email, err)
		}
