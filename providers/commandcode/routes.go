...
// CommandCode API key to /auth/commandcode/import and have the account land
// in the pool.
func init() {
	providers.RegisterAdminRoutes(map[string]providers.RouteHandler{
		"POST /auth/commandcode/import": importCommandCode,
	})
}

// importAccount mirrors the request body. Both `access_token` and a bare
// `user_…` key per line are accepted.
...
// importCommandCode handles POST /auth/commandcode/import. Accepts:
//   - a single importAccount JSON object
//   - a JSON array of importAccount
//   - newline-delimited bare API keys (one per line)
//
// Empty access tokens are rejected — CommandCode has no free / public tier.
func importCommandCode(host providers.Host, w http.ResponseWriter, r *http.Request) {
	...
	if len(entries) == 0 {
		writeJSONError(w, http.StatusBadRequest, "no commandcode entries found")
		return
	}
	...
	for i, e := range entries {
		...
		if err := host.RefreshAccountModels(&account); err != nil {
			logger.Warnf("[commandcode] model refresh failed for %s: %v", account.Email, err)
		}
...
// parseEntries accepts a single JSON object, a JSON array, or one bare API
// key per line (blank lines skipped).
