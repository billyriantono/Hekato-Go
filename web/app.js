...
(() => {
  ...
  function modalCodeBuddy(title, body) {
    title.textContent = t('modal.codebuddyTitle');
    body.innerHTML =
      '<p class="help-block">' + escapeHtml(t('modal.codebuddyDesc')) + '</p>' +
      '<div class="form-group"><label>' + escapeHtml(t('codebuddy.label')) + '</label>' +
      '<input type="text" id="codebuddyLabel" placeholder="' + escapeAttr(t('codebuddy.labelPlaceholder')) + '" /></div>' +
      '<div class="form-group"><label>' + escapeHtml(t('codebuddy.variant')) + '</label>' +
      '<select id="codebuddyVariant">' +
      '<option value="global">' + escapeHtml(t('codebuddy.global')) + '</option>' +
      '<option value="cn">' + escapeHtml(t('codebuddy.cn')) + '</option>' +
      '</select>' +
      '</div>' +
      '<div class="form-group"><label>' + escapeHtml(t('codebuddy.apiKey')) + ' <small>' + escapeHtml(t('local.tokenRequired')) + '</small></label>' +
      '<input type="password" id="codebuddyApiKey" class="font-mono" placeholder="sk-..." /></div>' +
      '<div class="modal-footer">' +
      '<button class="btn btn-secondary" data-modal-goto="add" type="button">' + escapeHtml(t('common.back')) + '</button>' +
      '<button class="btn btn-primary" id="importCodeBuddyBtn" type="button">' + escapeHtml(t('common.add')) + '</button>' +
      '</div>';
    $('importCodeBuddyBtn').addEventListener('click', importCodeBuddy);
  }
  ...
  // Import handlers
  async function importCodeBuddy() {
    const apiKey = $('codebuddyApiKey').value.trim();
    const label = $('codebuddyLabel').value.trim();
    const variant = $('codebuddyVariant').value;
    if (!apiKey) return toastWarning(t('codebuddy.apiKeyMissing'));
    const res = await api('/auth/codebuddy', { method: 'POST', body: JSON.stringify({ apiKey, label, variant }) });
    const d = await res.json();
    if (d.success) {
      closeModal(); loadAccounts(); loadStats();
      toastPrimary(t('codebuddy.importSuccess') + ': ' + (d.account?.email || d.account?.id));
  ...
  async function checkUpdate(manual) {
    ...
    try {
      ...
      const res = await fetch('https://raw.githubusercontent.com/billyriantono/Hekato-Go/main/version.json?t=' + Date.now());
